...
- **The payload belongs on stdin, never in argv.** A prompt passed as a command-line argument sits in `/proc/<pid>/cmdline` for anyone on the box to read, for as long as the process lives. Fine for a probe; wrong the moment the payload is a conversation, a document, or anything with a subject. `run_in_jail(..., stdin_payload=...)` exists for this.
- **A denyset that only knows one key order is half a denyset.** We scanned structured secrets serialized with sorted keys — while the code that builds prompts serialized in insertion order. The two strings look identical to a human and are different to `in`. `distinctive_forms()` now emits both, and clears authorized values in both.
- **"Detectable" is not "prevented".** Recording an input's hash in a run manifest makes a substitution *auditable afterwards*; it does not stop it. If a preregistration pins what an experiment consumes, compare and abort **before emitting anything** — `verify_pinned_input()`.
- **Hashing with a public salt is not opacity.** Source labels live in small, guessable universes (`patient_01`, `session_03_b`). If you salt them with a digest you publish, anyone recovers them by enumeration. Use an HMAC keyed with a secret that never enters the repo (`opaque_source_token()`), and fail closed when the key is missing.
- **Editing config is declaring an intention; the process is the fact.** A service kept serving the old mode for hours after its env file said otherwise, because nobody restarted it. Verify against `/proc/<pid>/environ` — and have the service publish its *effective* mode so the drift is visible at a glance.
- **A test that skips silently when its fixture is absent proves nothing.** An anti-ingestion check that `continue`s over missing files passes green in exactly the environment where it protects nothing. Materialize a controlled fixture, plant the violation, and assert the failure; gate the rest explicitly.
